...
- (void)addToolView{
    ...
    _toolView = [[UIView alloc] initWithFrame:CGRectMake(0, _boxView.frame.origin.y - 100, self.view.frame.size.width, 50)];
    _toolView.backgroundColor = [UIColor clearColor];
    
    CGFloat btnW = 50;
    CGFloat edge = (CGRectGetWidth(_toolView.frame) - 3 * btnW) / 4.0;
    
    UIButton *photoBtn = [[UIButton alloc] initWithFrame:CGRectMake(edge, 0, btnW, btnW)];
    [photoBtn setTitle:@"相册" forState:(UIControlStateNormal)];
    [photoBtn addTarget:self action:@selector(readerImageFromLibrary) forControlEvents:(UIControlEventTouchUpInside)];
    
    UIButton *torchBtn = [[UIButton alloc] initWithFrame:CGRectMake(2 * edge + btnW, 0, btnW, btnW)];
    [torchBtn setTitle:@"灯光" forState:(UIControlStateNormal)];
    [torchBtn addTarget:self action:@selector(turnTorch:)   forControlEvents:UIControlEventTouchUpInside];
    
    UIButton *closeBtn = [[UIButton alloc] initWithFrame:CGRectMake(edge * 3 + 2 * btnW, 0, btnW, btnW)];
    [closeBtn setTitle:@"关闭" forState:(UIControlStateNormal)];
    [closeBtn addTarget:self action:@selector(close) forControlEvents:(UIControlEventTouchUpInside)];
    
    [_toolView addSubview:photoBtn];
    [_toolView addSubview:torchBtn];
    [_toolView addSubview:closeBtn];
...
- (void)turnTorch:(UIButton *)btn{
    ...
    if (device.hasTorch) {
        ...
        if (!b) {
            if (error) {
                NSLog(@"打开闪光灯失败:%@", error.localizedDescription);
            }
            return;
        }
        device.torchMode =
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    NSString *result = [self scQRReaderForImage:srcImage];
